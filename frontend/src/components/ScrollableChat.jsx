                    ...
                    {m.grpAddEvent ? (
                        <div style={{ display: "flex", justifyContent: "center" }}>
                            <p style={{ textAlign: "center", backgroundColor: "#f0f0f0", padding: "10px" }}>
                                <p>{m.chat.groupAdmin._id === user._id ? "You created this group." : `You were added by ${m.chat.groupAdmin.name}`}</p>
                                {
                                    m.groupUsers.map((u) => (
                                        <p key={u._id}>
                                            {u._id !== user._id
                                                ? (m.chat.groupAdmin.name === user.name
                                                    ? `You added ${u.name}`
                                                    : `${m.chat.groupAdmin.name} added ${u.name}`)
                                                : ''}
                                        </p>
                                    ))
                                }
                                        ...
                                        {reply.file && (
                                            <Tooltip label="Click to download" placement="top">
                                                {reply.file.endsWith('.pdf') ? (
                                                    <a onClick={() => downloadFileWithName(reply.file)}>.pdf</a>
